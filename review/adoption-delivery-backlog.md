...
| ADP-03 | Data handling and provider applicability decision / Calcu + deployment owner | in_progress | Owner selected personal ChatGPT login, declined synthetic-only scope and requested no extra provider-retention constraint. Remaining work is mandatory source classification, runtime/agent retention, Grant projection and applicable-path evidence, not another generic privacy preference. The [pinned CLI audit](../reference/adoption/calcu/retention_probe/source-audit.md) identifies a SQLite tool-response persistence path; transient capability remains unestablished, with no live run. Unknown capabilities remain unknown. |
