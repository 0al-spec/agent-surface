...
> [!NOTE]
> This is the authoritative Core module selected by the ASP Document Set
...
- [Authors' Contact Information](modules/core.md#authors-contact-information)
- [Status of this Memo](modules/core.md#status-of-this-memo)
- [Copyright Notice and Licensing](modules/core.md#copyright-notice-and-licensing)
- [Abstract](modules/core.md#abstract)
- [Normative and Informative Sections](modules/core.md#normative-and-informative-sections)
- [Motivation](modules/core.md#motivation)
- [Goals](modules/core.md#goals)
- [Non-Goals](modules/core.md#non-goals)
- [Conventions](modules/core.md#conventions)
- [Terminology](modules/core.md#terminology)
- [Design Principles](modules/core.md#design-principles)
- [Relationship to Existing Protocols](modules/core.md#relationship-to-existing-protocols)
- [ASP-over-MCP Binding Profile](modules/bindings/asp-over-mcp.md#asp-over-mcp-binding-profile)
- [Conceptual Architecture](modules/core.md#conceptual-architecture)
- [Protocol Layers](modules/core.md#protocol-layers)
- [Modular RFC Publication Architecture](modules/core.md#modular-rfc-publication-architecture)
- [Canonical Integrity and Provenance](modules/evidence.md#canonical-integrity-and-provenance)
- [Agent Surface Manifest](modules/core.md#agent-surface-manifest-1)
- [Action Execution Model](modules/safe-effects.md#action-execution-model)
- [Risk Taxonomy](modules/safe-effects.md#risk-taxonomy)
- [Effect Model](modules/safe-effects.md#effect-model)
- [Approval Semantics](modules/safe-effects.md#approval-semantics)
- [Idempotency](modules/safe-effects.md#idempotency)
- [Pluggable Agent Identity Evidence Profile](modules/authorization.md#pluggable-agent-identity-evidence-profile)
- [Minimal Agent Passport Grant-Issuance Profile](modules/authorization.md#minimal-agent-passport-grant-issuance-profile)
- [Runtime Identity Profile](modules/authorization.md#runtime-identity-profile)
- [Remote Processing Privacy Profile](modules/privacy.md#remote-processing-privacy-profile)
- [Agent Training Use Policy Profile](modules/privacy.md#agent-training-use-policy-profile)
- [Runtime Attestation Optional Profile](modules/authorization.md#runtime-attestation-optional-profile)
- [Agent Grant](modules/authorization.md#agent-grant)
- [Purpose- and Task-Bound Agent Grant Profile](modules/authorization.md#purpose-and-task-bound-agent-grant-profile)
- [Capability Matching](modules/authorization.md#capability-matching)
- [Observability Context](modules/evidence.md#observability-context)
- [Sessions and Actions](modules/core.md#sessions-and-actions)
- [Receipts](modules/evidence.md#receipts)
- [Portable Replay Bundle Profile](modules/evidence.md#portable-replay-bundle-profile)
- [Revocation Semantics](modules/authorization.md#revocation-semantics)
- [Error Model](modules/core.md#error-model)
- [Versioning and Compatibility](modules/core.md#versioning-and-compatibility)
- [Security Considerations](modules/core.md#security-considerations)
- [Privacy Considerations](modules/privacy.md#privacy-considerations)
- [Conformance](modules/conformance.md#conformance)
- [Application MVP Mapping](modules/conformance.md#application-mvp-mapping)
- [Example End-to-End Flow](modules/conformance.md#example-end-to-end-flow)
- [Open Questions](modules/core.md#open-questions)
- [References](modules/core.md#references)
- [Appendix A: Why This Is Not Just an API Token](modules/core.md#appendix-a-why-this-is-not-just-an-api-token)
- [Appendix B: Why This Is Not Just Computer Use](modules/core.md#appendix-b-why-this-is-not-just-computer-use)
- [Appendix C: Product Positioning](modules/core.md#appendix-c-product-positioning)
...
The current catalog uses `modular`. In that mode:

- all seven selected module sources are active and canonical;
- `drafts/agent-surface.md` is generated from those sources and has no
  independent normative authority;
- `reserved_documents` is empty;
- namespace and registry ownership is assigned to exact active documents; and
- the build manifest, source map, aggregate digest, and selected source
  digests MUST agree before the document set can be published.

The former `transitional_monolith` catalog remains historical evidence of the
pre-activation state. It MUST NOT be combined with sources or provenance from
the active modular document set.
...
The initial active modular graph contains Core, Authorization, Safe Effects,
Evidence, Privacy, the ASP-over-MCP binding, and Conformance. The modular v1
catalog contains no reservations. A future catalog version can define
post-activation reservation and multi-version selection rules; this one does
not. Later document sets MAY add or omit extensions and bindings when their
exact dependency closure remains valid.
...
selected target. The activation relocation record covers every public anchor
formerly owned by the monolith, and the publication resolver validates both
the historical tuple and its selected Core target.

The historical monolith predates this rule and contains derived aggregate
...
ordinal convention is a valid source of new public ids. The modular activation
preserves every still-referenced legacy aggregate form through the validated
relocation table. A legacy aggregate fragment is a compatibility reference,
not a new document-scoped exported anchor.
...
The v1 validator accepts `modular` mode only when it validates Hyperprompt
provenance, complete source-map coverage, output and source digests,
cross-document relocation records, and atomic readiness. A compiler success
without those checks is not a valid publication.

<a id="atomic-modular-activation"></a>
### Atomic Modular Activation

The transition from `transitional_monolith` to `modular` was one atomic
document-set transition. A modular document set is valid only when:
...
Before a future activation, non-authoritative candidate sources MAY be prepared under a
...
candidate selected for a modular set is complete. The atomic transition moves
all selected candidates to their canonical target paths, removes every
reservation, selects them as active documents, builds and validates the
aggregate and provenance, and removes the previous authority source from active
selection in the same source state. No incremental module extraction can claim
active modular authority before that point.
